...
SCRIPT_PATH="/share/project/liuze/projects/contextual_mmemb/code/baseline_retrievers/BGE-VL/eval_bgevl_contextual.py"
    ...
    for ((i=0; i<NUM_GPUS; i++)); do
        ...
        (
            for ((j=i; j<${#SOURCES[@]}; j+=NUM_GPUS)); do
                ...
                CUDA_VISIBLE_DEVICES=$GPU_ID python "$SCRIPT_PATH" --source "$SOURCE" --base_data_dir "$BASE_DIR" --batch_size 16 --k 100
            done
        ) &
        pids+=($!)
    done
