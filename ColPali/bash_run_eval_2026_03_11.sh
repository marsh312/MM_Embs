...
SCRIPT_PATH="/share/project/liuze/projects/contextual_mmemb/code/baseline_retrievers/ColPali/eval_colpali_contextual.py"
    ...
    for ((i=0; i<NUM_GPUS; i++)); do
        ...
        (
            for ((j=i; j<${#SOURCES[@]}; j+=NUM_GPUS)); do
                ...
                CUDA_VISIBLE_DEVICES=$GPU_ID python "$SCRIPT_PATH" --source "$SOURCE" --base_data_dir "$BASE_DIR" --model_path "$MODEL_PATH"
            done
        ) &
        pids+=($!)
    done
