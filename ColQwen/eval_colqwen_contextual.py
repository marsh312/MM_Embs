...
from colqwen_dataset import ColQwenQueryDataset, ColQwenCorpusDataset
from colqwen_model import ColQwenEmbed
...
def main():
    ...
    parser.add_argument("--model_path", type=str, default="/share/project/shared_models/colqwen2.5-v0.2")
    ...
    output_dir = os.path.join(base_data_dir.replace("/testdatas", "/testdatas_results"), "ColQwen", args.source)
    
    # 1. Init Model
    model = ColQwenEmbed(model_path=args.model_path)
    ...
    try:
        ...
        query_ds = ColQwenQueryDataset(raw_queries)
        corpus_ds = ColQwenCorpusDataset(raw_corpus)

        # 2. Encode Queries (Text)
        print("Encoding Queries...")
        # Note: ColQwenEmbed.get_query_embeddings expects list of strings
        ...
        # Note: ColQwenEmbed.get_image_embeddings expects list of image paths/objects
