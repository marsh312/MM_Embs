...
def main():
    ...
    parser.add_argument("--output_root", type=str, default="/share/project/liuze/projects/contextual_mmemb/results/Qwen3VL-Emb")
    ...
    parser.add_argument("--tasks", nargs='+', default=None, help="Specify benchmark name keywords, e.g.: FinRAG SlideVQA")
    ...
    for task in tasks:
        try:
            ...
            if args.score_method == 'image':
                ...
                c_embs = get_embeddings(model, corpus_inputs, batch_size=args.batch_size, desc="Encoding Text Corpus")
                ...
                corpus_inputs_img = []
                for item in corpus_ds:
                    corpus_inputs_img.append({
                        "image": item['image'],
                        "instruction": "Represent the given image."
                    })
                c_embs_img = get_embeddings(model, corpus_inputs_img, batch_size=args.batch_size, desc="Encoding Image Corpus")
                
                print("  Encoding Context part...")
                corpus_inputs_ctx = []
                for item in corpus_ds:
                    corpus_inputs_ctx.append({
                        "text": item['text'],
                        "instruction": "Represent the given document context."
                    })
                c_embs_ctx = get_embeddings(model, corpus_inputs_ctx, batch_size=args.batch_size, desc="Encoding Text Corpus")
                ...
                c_embs = get_embeddings(model, corpus_inputs, batch_size=args.batch_size, desc="Encoding Text+Image Corpus")
